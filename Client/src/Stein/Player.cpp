...
{
	_frameBuffer.DrawRect(_position.x * scale - size / 2, _position.y * scale - size / 2, size, size, color);
	_frameBuffer.DrawLine(_position.x * scale, _position.y * scale, _position.x * scale + 15 * _direction.x, _position.y * scale + 15 * _direction.y, 3, color);
}
